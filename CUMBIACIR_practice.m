...
function output_param = CUMBIACIR_practice(ii)
...
D       = param(4,ii);                         % section diameter (mm)
clb     = param(5,ii);                         % cover to longitudinal bars (mm)

% member properties

L          = param(6,ii);                      % member clear length (mm)

bending_id = param(7,ii);                      % single or double
...
ductilitymode_id = param(8,ii);                % biaxial or uniaxial
...
nbl     = param(9,ii);                        % number of longitudinal bars
Dbl     = param(10,ii);                       % long. bar diameter (mm)   
Dh      = param(11,ii);                       % diameter of transverse reinf. (mm)
type_id = param(12,ii);                       % 'spirals' or 'hoops'*
...
fpc     = param(15,ii);                 % concrete compressive strength (MPa)
Ec      = param(16,ii);                 % concrete modulus of elasticity (MPa) or
...
esm     = param(17,ii);                 % max transv. steel strain (usually ~0.10-0.15)*
espall  = 0.0064;                       % max uncon. conc. strain (usually 0.0064)

fy      = param(18,ii);                 % long steel yielding stress (MPa)
fyh     = param(19,ii);                 % transverse steel yielding stress (MPa)
Es      = param(20,ii);                 % steel modulus of elasticity
fsu     = param(21,ii);                 % long steel max stress (MPa)*
esh     = param(22,ii);                 % long steel strain for strain hardening (usually 0.008)*
esu     = param(23,ii);                 % long. steel maximum strain (usually ~0.10-0.15)*
...
if Ec==0
    Ec = 4700*(fpc^(1/2));                   % concrete modulus of elaticity
end
...
Vs    = (0.5*pi*(0.25*pi*(Dh^2))*fyh*cot((30/180)*pi)*(D-clb+0.5*Dh-cMn)/s)/1000;
