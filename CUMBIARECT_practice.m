...
function output_param = CUMBIARECT_practice(ii)
...
% read excel file
pkg load io
param = xlsread ('input_columns_RECT.xlsx', 'input', 'B1:ZZZ50');                    
                    
% section properties:

H = param(4,ii);                               % section height (mm)- perp to x
B = param(5,ii);                               % section width  (mm)- perp to y
ncx = param(6,ii);                             % # legs transv. steel x_dir (confinement)
ncy =  param(7,ii);                            % # legs transv. steel y_dir (shear)
clb = param(8,ii);                             % cover to longitudinal bars (mm)

% member properties

L = param(9,ii);                               % member clear length (mm)

bending_id = param(10,ii);                     % single or double
...
ductilitymode_id = param(11,ii);               % biaxial or uniaxial

if ductilitymode_id == 1  
    ductilitymode = 'uniaxial';            
elseif ductilitymode_id == 2 
...
MLR = reshape(param(27:27+mm*nn-1,ii),mm,nn);

% transverse reinforcement details

Dh = param(12,ii);                             % diameter of transverse reinf. (mm)
s = param(13,ii);                              % spacing of transverse steel (mm)*

% applieed loads:

P = param(14,ii);                              % axial load kN (-) tension (+)compression
...
wi = param(15,ii);                                % Average clear distances between
                                                  % periferical longitudinal bars properly
                                                  % restrained (used only if the mander model is selected)
% material properties 

fpc = param(16,ii);                % concrete compressive strength (MPa)
Ec = param(17,ii);                 % concrete modulus of elasticity (MPa) or
...
esm = param(18,ii);                % max transv. steel strain (usually ~0.10-0.15)*
espall  = 0.0064;                  % max uncon. conc. strain (usually 0.0064)

fy = param(19,ii);                 % long steel yielding stress (MPa)
fyh = param(20,ii);                % transverse steel yielding stress (MPa)
Es = param(21,ii);                 % steel modulus of elasticity
fsu = param(22,ii);                % long steel max stress (MPa)*
esh = param(23,ii);                % long steel strain for strain hardening (usually 0.008)*
esu = param(24,ii);                % long. steel maximum strain (usually ~0.10-0.15)*
...
% Shear strength information
theta   = 30;           % the critical diagonal tension crack angle for assessment (30 degrees Priestley et al. default)
thetaD  = 35;           % the critical diagonal tension crack angle for design (35 degrees Priestley et al. default)
phiS    = 1;            % Strength reduction factor for shear (assessment)
...
addpath('models')                  % directory with the user specified material models
...
Vs    =(ncy*0.25*pi*(Dh^2)*fyh*(H-clb+0.5*Dh-cMn)*cot((theta/180)*pi)/s)/1000;
Vsd   =(ncy*0.25*pi*(Dh^2)*fyh*(H-clb+0.5*Dh-cMn)*cot((thetaD/180)*pi)/s)/1000;
beta  = min(0.5+20*LongSteelRatio,1);
Dductf = displ/dyf;
...
output_param(1)  = min(V);
output_param(2)  = max(V);
output_param(3)  = failforce;
output_param(4)  = faildispl;
output_param(5)  = fyM;
output_param(6)  = Mn;
output_param(7)  = eqcurv;
output_param(8)  = dy;
output_param(9)  = interp1(coverstrain,displ,0.004);
output_param(10) = max(displbilin);
output_param(11) = fy;
